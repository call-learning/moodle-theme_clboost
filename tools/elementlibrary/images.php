...
echo html_writer::tag('p',
    'Generally the component should be "moodle" for existing moodle images, "totara_core" for core '.
    'totara images, "totara_modname" for module specfic images. It is possible to specify a theme image directly,'.
    ' but this should be avoided unless the image is only used by the theme - normally the theme would just override'.
    ' the core or module image if required');
...
echo html_writer::tag('p',
    'This will set the title to be the same as the alt text, and by default assign the "smallicon" class '.
    'unless you set any other class via the attributes.');

echo html_writer::tag('p',
    'If you want to create a linked icon use $OUTPUT->action_icon(). Note that action_icon requires a pix_icon'.
    ' object, not the rendered string so make sure the second argument is "new pix_icon()" not $OUTPUT->pix_icon().');
...
echo html_writer::tag('p',
    'action_icon() takes an option linktext boolean argument for putting the alt text next to the icon like this.'.
    ' You can also trigger javascript actions using the component_action argument.');
$url = new moodle_url('index.php');
$icon = new pix_icon('t/add', 'Add');
...
echo html_writer::tag('p',
    'There is a special class for rendering emoticons. All it does dfiferently is set "emoticon" as the default class.');
...
echo html_writer::tag('p',
    'One final comment regarding images - you can reference moodle images in style sheets,'.
    ' just use this syntax and it will be substituted with the correct path when the stylesheet is compiled.');
